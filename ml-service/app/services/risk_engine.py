...
def compute_module_risk(df: pd.DataFrame, filtered_rules: list[dict]) -> list[dict]:
    ...
    for module, raw_score in raw_risks.items():
        ...
        # Safeguards: clamp to 0-100 range and log warning if abnormal
        if norm_score < 0.0 or norm_score > 100.0:
            logger.warning(f"Abnormal raw normalized risk score calculated: {norm_score:.2f} for module '{module}'. Clamping.")
        clamped_score = max(0.0, min(100.0, norm_score))
        normalized_risks[module] = round(clamped_score, 2)
    ...
    for module in unique_modules:
        ...
        # User defined thresholds: Low < 30, Medium < 60, High < 80, Critical 80-100
        if score < 30.0:
            level = "LOW"
        elif score < 60.0:
            level = "MEDIUM"
